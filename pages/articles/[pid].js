...
import {
  scrollToTop,
  scrollToArticle,
} from "../../components/utils/functions/scrollToTop";
  ...
  return (
    <>
      <Head>
        ...
        <meta
          property="og:url"
          content={`http://www.billpacello.com/articles/${id}`}
        />
      ...
      <Container className={`container-body`} style={{ paddingBottom: "50px" }}>
        <Row style={{ justifyContent: "center" }}>
          <RenderArticles articles={posts_} />
        </Row>
        <hr className="hr0" />
        <hr className="hr1" />
        ...
        <Row>
          <DisplayArticle article={currentPost} />
        </Row>
      </Container>
    </>
  );
...
export const RenderArticles = ({ articles }) => {
  return articles.map((a, i) => {
    ...
    return (
      <Link key={title} href={`/articles/${i + 1}`} as={`/articles/${i + 1}`}>
        <div
          key={title}
