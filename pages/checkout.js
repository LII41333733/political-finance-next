...
import {
  ContentfulClient,
  getLessons,
} from "../components/utils/contentfulOptions";
...
export default () => {
            ...
            purchase_units: [
              {
                ...
                amount: {
                  currency_code: "USD",
                  value: donatorValue || action.price || 3.5,
                },
              },
            ],
          });
  ...
  const purchaseConfirmation = (details) => {
    ...
    if (action === "donation") {
      return (
        <div className="nav-link-confirm">
          ...
          <h3>{`Amount: $${donatorValue.toFixed(2)}`}</h3>
          <br />

          <Link href={"/home"}>
            <Button className="mb-4" size="lg" variant="dark">
              {"Return to Home"}
            </Button>
          </Link>
        </div>
    ...
    return (
      <div className="nav-link-confirm">
        ...
        <h3>{`Confirmation ID: ${orderDetails.id}`}</h3>
        <h3>{`Date & Time: ${parsedDate} ${parsedTime}`}</h3>
        <h3>{`Amount: $${details.price || "3.50"}`}</h3>
        <br />
    ...
    >
      {action === "donation" ? (
        <h3 className="schadow">
          <strong>Select Payment</strong> - Complete your donation with <br />{" "}
          either of the below options:
        </h3>
      ) : (
        <h3 className="schadow">
          <strong>Select Payment</strong> - Complete your purchase with <br />{" "}
          either of the below options:
        </h3>
      )}
      <Button
      ...
      <Container className="container-body">
        <Row>
          <Col lg={6} className={"mx-auto"}>
            <Card border={"dark"} className={`p-3 my-3 ml-5`}>
              <div className="p-0">
                <h3 className="schadow">
  ...
  const purchaseComponent = () => (
    <Container className="container-body">
      <Row>
        <Col lg={6} className={`mx-auto`}>
          {showConfirmation ? purchaseConfirmation(action) : renderPaypalCard()}
