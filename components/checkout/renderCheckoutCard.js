...
export default ({ data, showConfirmation }) => {
  const { image, href, media, title, price } = data;
  console.log(data);
  return (
    <>
      <Card
      ...
      >
        ...
        <Card.Body className="p-0 my-auto">
          <Card.Header className="align-items-center justify-content-center d-flex">
            ...
            <Row>
              <Col>
                <Button
                  variant={"dark"}
                  size="lg"
                  className={`checkoutBtnDownload w-100 ${
                    !showConfirmation && "not-allowed"
                  }`}
                  href={media}
                  download={title}
